...
import { 
  ...
  Chip,
  Switch,
  Menu // <--- NUEVO: Para el menú desplegable
...
export const AdminScreen = () => {
  ...
  const [menuVisible, setMenuVisible] = useState(false); // <--- ESTADO DEL MENÚ
  ...
  // --- 🛠️ LÓGICA DE NAVEGACIÓN INTERNA ---
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <Surface style={[styles.header, { backgroundColor: theme.background }]} elevation={1}>
        ...
        <View style={styles.headerCentral}>
            ...
            <Text style={styles.subtituloHeader}>{vista === 'lista' ? 'STOCK ACTUAL' : vista.toUpperCase()}</Text>
        </View>

        {/* EL MENÚ SE DISPARA DESDE EL ÍCONO DE CARTERA */}
        <Menu
          visible={menuVisible}
            ...
            <Card style={[styles.orderCard, { borderLeftColor: item.estado === 'Pendiente' ? theme.primary : '#25D366' }]}>
              <Card.Content>
                ...
                {item.items?.map((prod: any, idx: number) => (
                  <Text key={idx} style={{ fontSize: 13 }}>• {prod.nombre} (x{prod.quantity})</Text>
                ))}
